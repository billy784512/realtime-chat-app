...
const a_logout = document.querySelector("#logout");
  ...
  try{
    ...
    result[0].forEach(user => {
      ...
      btn.addEventListener("click", async() => {
        if(user[0] == user_info.username){
          window.alert("You cannot chat with yourself.")
          return
        }
        const newRoom = await createRoom({username: user[0]});
        window.location.href = `./chat.html?userid=${user_info._id}room_id=${newRoom.room_id}`;
      })
    })
  }catch(error){
    console.log(error);
...
a_logout.addEventListener("click", async() => {
  now = new Date();
  document.cookie = `jwt=; expires=${now}; path=/;`;;
  window.location.href = "./login.html";
})
